...
const resolvePromise: Promise<string> = new Promise((resolve, reject) => {
  setTimeout(() => {
    resolve("resolve");
  }, 2000);
});

const rejectPromise: Promise<never> = new Promise((resolve, reject) => {
  setTimeout(() => {
    reject("reject");
  }, 2000);
});
...
function fetchDataFromUrl(url: string): Promise<any> {
...
async function logPromiseResolution(promise: Promise<any>): Promise<void> {
  try {
...
async function handleRejectedPromise(promise: Promise<any>): Promise<void> {
  try {
    const result = await promise;
    console.log("Resolved value: ", result);
  } catch (error) {
...
const sampleResolvedPromise: Promise<string> = new Promise(
  (resolve, reject) => {
    setTimeout(() => {
...
const sampleRejectedPromise: Promise<never> = new Promise((resolve, reject) => {
  setTimeout(() => {
...
function logApiResponseWithPromises(): void {
  fetch(apiURLs[0])
    .then((response) => {
      if (!response.ok) {
        throw new Error("Network response was not ok");
...
async function fetchAndLogApiResponseWithAsync(): Promise<void> {
  try {
...
function fetchAllDataFromUrls(url: string): Promise<any> {
...
const fetchPromisesAll: Promise<any>[] = apiURLs.map((url) =>
...
const fetchPromisesRace: Promise<any>[] = apiURLs.map((url) =>
  fetchDataFromUrl(url),
);
